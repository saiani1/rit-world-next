...
serve(async (req: Request) => {
  ...
  try {
    ...
    const prompt = `
      ...
      중요: 출력 길이가 길어지더라도, 각 질문과 답변의 내용은 절대 요약하거나 생략하지 말고 대화 내용 화면에서 확인할 수 있는 원문 스크립트 그대로 전체를 추출하여 JSON에 담으세요.
      ...
      5. 대화 내용을 분석하여 질문과 답변 쌍으로 분리하고 qa_data 배열에 담으세요. **이때, 어떠한 질문이나 답변도 임의로 요약, 축약, 누락시키지 마세요. 스크립트 상의 모든 문답을 100% 원문 그대로 추출해야 합니다.** (가벼운 아이스브레이킹 질문이나 사소한 잡담성 문답도 모두 포함하세요.)
      6. AI의 주관적인 판단으로 내용을 편집하지 말고 본래 텍스트를 최대한 살려서 기재하세요.
      7. 각 항목이 면접관의 질문인지, 지원자의 역질문인지 판단하여 question_type 필드에 **반드시** '면접관' 또는 '역질문' 중 하나로 **한국어로** 표기하세요.
      8. "면접관" 유형일 경우:
         - question_jp: 면접관의 질문
         - answer_jp: 지원자의 답변
         - feedback_ko: 지원자의 답변에 대한 피드백 (한국어)
         - best_answer_jp: 더 나은 답변 제안 (**오직 순수 일본어로만 작성하세요. 절대 한국어 해석이나 부연 설명을 덧붙이지 마세요.**)
      9. "역질문" 유형일 경우:
         - question_jp: 지원자의 질문
         - answer_jp: 면접관의 답변
         - feedback_ko: 지원자의 질문이 적절했는지에 대한 피드백 (한국어)
         - best_answer_jp: 더 정중하거나 좋은 질문 표현 제안 (**오직 순수 일본어로만 작성하세요. 절대 한국어 해석이나 부연 설명을 덧붙이지 마세요.**)
      10. 모든 한국어 번역 필드(_ko)를 채우세요.
